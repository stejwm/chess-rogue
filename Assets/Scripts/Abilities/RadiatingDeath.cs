...
public class RadiatingDeath : Ability
{
    ...
    public void RadiateDeath(Chessman attacker, Chessman defender){
        if (defender == piece)
        {
            board.AbilityLogger.AddAbilityLogToQueue($"<sprite=\"{piece.color}{piece.type}\" name=\"{piece.color}{piece.type}\"><color=white><gradient=\"AbilityGradient\">Radiating Death</gradient></color>", $"<color=red>-1</color> to all stats on {BoardPosition.ConvertToChessNotation(defender.xBoard, defender.yBoard)}");
