    ...
    public void PieceAdded(Chessman addedPiece){
        if(addedPiece.owner==piece.owner && addedPiece.type==PieceType.Knight){ 
            if(!appliedBonus.ContainsKey(addedPiece)){
                ...
                bonus++;
                board.AbilityLogger.AddAbilityLogToQueue($"<sprite=\"{piece.color}{piece.type}\" name=\"{piece.color}{piece.type}\"><color=white><gradient=\"AbilityGradient\">General</gradient></color>", "bonus increased to " + bonus); 
            }
        }
    }
